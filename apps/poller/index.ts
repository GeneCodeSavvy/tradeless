...
const asssets = [
	{ symbol: "SOL_USDC", asset: "SOL", decimal: 6 },
	{ symbol: "ETH_USDC", asset: "ETH", decimal: 4 },
	{ symbol: "BTC_USDC", asset: "BTC", decimal: 4 },
];
...
ws.on("message", async (data) => {
	...
	const formatted_data: {
		ask: any;
		bid: any;
		symbol: "SOL_USDC" | "ETH_USDC" | "BTC_USDC";
	} = {
	...
	await redis.xAdd(TICKER_STREAM_KEY, "*", {
		data: JSON.stringify(formatted_data),
	});
	const mid_price =
		(parseFloat(formatted_data.ask) + parseFloat(formatted_data.bid)) / 2;
	const { asset, decimal } = asssets.find(
		(a) => a.symbol === formatted_data.symbol,
	)!;
...
setInterval(async () => {
	...
	if (payload.price_updates.length > 0) {
		await redis.publish(PRICE_CHANNEL, JSON.stringify(payload));
		console.log("Added data to the stream: ", payload);
	}
